...
class MCPServerRegistry:
    ...
    The registry uses the database as the source of truth for server configurations,
    and delegates the actual process management to the MCPServerManager.
    """
    ...
    async def start_enabled_servers(self) -> None:
        ...
        async with AsyncSessionLocal() as db:
            ...
            for server in enabled_servers:
                ...
                if not await self.manager.get_session(server.name):
                    try:
                        ...
                        await self.manager.start_server(server_name=server.name, config=server_config)
    ...
    async def update_server_config(self, db: AsyncSession, name: str, config: MCPServerBase) -> None:
        ...
        session = await self.manager.get_session(name)
        if session and not config.enabled:
            # Shutdown server if it's now disabled
            await self.manager.shutdown_server(name)
        elif not session and config.enabled:
            # Start server if it's now enabled
            try:
                await self.manager.start_server(server_name=name, config=config)
    ...
    async def get_running_servers(self) -> dict[str, ClientSession]:
        ...
        This uses the in-memory state from the manager rather than the database,
